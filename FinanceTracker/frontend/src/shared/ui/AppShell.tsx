...
export function AppShell() {
  ...
  const headerBg = theme === "dark" ? "#042d22" : "#edece0";
  const headerText = theme === "dark" ? "#e6ff55" : "#050315";
  const siderBg = theme === "dark" ? "#0d4132" : "#dddbce";

  return (
    <Layout style={{ minHeight: "100vh" }}>
      <Sider width={250} className="ft-app-sider" theme={theme === "dark" ? "dark" : "light"} style={{ background: siderBg }}>
        <div style={{ padding: 20 }}>
          <Typography.Title level={4} style={{ margin: 0, color: theme === "dark" ? "#e6ff55" : "#050315" }}>
            FinanceTracker
          </Typography.Title>
        </div>
        <Menu
          ...
          items={[
            { key: "/", icon: <HomeOutlined />, label: <Link to="/">{t("nav.dashboard")}</Link> },
            { key: "/accounts", icon: <BankOutlined />, label: <Link to="/accounts">{t("nav.accounts")}</Link> },
            { key: "/transfer", icon: <SwapOutlined />, label: <Link to="/transfer">{t("nav.transfer")}</Link> },
            { key: "/transfer/history", icon: <HistoryOutlined />, label: <Link to="/transfer/history">{t("nav.transferHistory")}</Link> },
            { key: "/transactions", icon: <ProfileOutlined />, label: <Link to="/transactions">{t("nav.transactions")}</Link> },
            { key: "/categories", icon: <TagsOutlined />, label: <Link to="/categories">{t("nav.categories")}</Link> },
            { key: "/budgets", icon: <WalletOutlined />, label: <Link to="/budgets">{t("nav.budgets")}</Link> },
            { key: "/analytics", icon: <PieChartOutlined />, label: <Link to="/analytics">{t("nav.analytics")}</Link> },
            { key: "/recurring-payments", icon: <FundOutlined />, label: <Link to="/recurring-payments">{t("nav.recurringPayments")}</Link> },
            { key: "/receipts", icon: <ReadOutlined />, label: <Link to="/receipts">{t("nav.receipts")}</Link> },
            { key: "/subscriptions", icon: <CrownOutlined />, label: <Link to="/subscriptions">{t("nav.subscriptions")}</Link> },
      ...
      <Layout>
        <Header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingInline: 20, background: headerBg, borderBottom: theme === "dark" ? "1px solid #13ae87" : "1px solid #d1d4c4" }}>
